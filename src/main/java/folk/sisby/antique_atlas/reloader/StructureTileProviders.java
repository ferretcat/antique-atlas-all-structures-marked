...
public class StructureTileProviders extends JsonDataLoader implements IdentifiableResourceReloadListener {
    ...
    private final Map<ProviderType, Pair<Map<Identifier, StructureTileProvider>, Map<Identifier, MarkerTexture>>> PROVIDER_MAPS = Map
            .of(
    ...
    public Map<ChunkPos, TileTexture> resolve(Map<ChunkPos, TileTexture> outTiles,
            Map<ChunkPos, StructureTileProvider> structureProviders, Map<ChunkPos, String> tilePredicates,
            StructurePieceSummary piece, World world) {
        if (piece instanceof JigsawPieceSummary jigsawPiece) {
            if (pieceJigsawSingleTiles.containsKey(jigsawPiece.getId())) {
                StructureTileProvider provider = (jigsawPiece
                        .getElementType() == StructurePoolElementType.FEATURE_POOL_ELEMENT ? pieceJigsawFeatureTiles
                                : pieceJigsawSingleTiles)
                        .get(jigsawPiece.getId());
                provider.getTextures(world, jigsawPiece.getBoundingBox(), jigsawPiece.getJunctions(), tilePredicates)
                        .forEach((pos, texture) -> {
                            if (structureProviders.containsKey(pos)
                                    && structureProviders.get(pos).priority() < provider.priority())
                                return;
    ...
    public void resolve(Map<ChunkPos, TileTexture> outTiles, Map<ChunkPos, StructureTileProvider> structureProviders,
        ...
        if (startMarkers.containsKey(key.getValue())) {
            ...
            tagMarkers.entrySet().stream()
                    ...
                    .ifPresent(entry -> {
                        outMarkers.put(
                                new AtlasStructureLandmark(pos.getCenterAtY(0), ProviderType.TAG, entry.getKey(), null),
                                entry.getValue());
                    });
        }
        ...
        if (startTiles.containsKey(key.getValue())) {
            ...
            provider.getTextures(world, summary.getBoundingBox(), debugPredicates).forEach((pos2, texture) -> {
                if (structureProviders.containsKey(pos) && structureProviders.get(pos).priority() < provider.priority())
                    return;
    ...
    protected void apply(Map<Identifier, JsonElement> prepared, ResourceManager manager, Profiler profiler) {
        ...
        Set<TileTexture> unusedTextures = new HashSet<>(
                textures.values().stream().filter(t -> t.id().getPath().startsWith("structure")).toList());
            ...
            PROVIDER_MAPS.forEach((providerType, pair) -> {
                ...
                if (fileId.getPath().startsWith(providerType.prefix())) {
                    Identifier id = Identifier.of(fileId.getNamespace(),
                            fileId.getPath().substring(providerType.prefix().length()));
                    try {
                        ...
                        if (fileJson.has("textures")) {
                            ...
                            if (defaultTextures != null) {
                                StructureTileProvider provider = new StructureTileProvider(id, priority,
                                        defaultTextures);
                                ...
                                for (String matcherKey : textureObject.keySet()) {
                                    ...
                                    StructureTileProvider.ChunkMatcher matcher = StructureTileProvider
                                            .getChunkMatcher(matcherId);
                                    if (matcher == null)
                                        throw new IllegalStateException(
                                                "Matcher %s does not exist!".formatted(matcherId.toString()));
                                    List<TileTexture> matcherTextures = resolveTextureJson(textures,
                                            textureObject.get(matcherKey));
                                    if (matcherTextures == null)
                                        throw new IllegalStateException("Malformed object %s in textures object!"
                                                .formatted(matcherId.toString()));
                                    matcherTextures.forEach(unusedTextures::remove);
                                    matchers.put(matcher, matcherTextures);
                                }
                                if (matchers.isEmpty()) {
                                    throw new IllegalStateException(
                                            "No matcher keys were found in the textures object!");
                                }
                        ...
                        if (fileJson.has("markers")) {
                            ...
                            if (texture == null)
                                throw new IllegalStateException(
                                        "Marker texture %s does not exist!".formatted(markerTextureId));
                        ...
                        AntiqueAtlas.LOGGER.error("[Antique Atlas] Error reading structure tile provider {}!", fileId,
                                e);
                    }
                }
            });
        ...
        for (TileTexture texture : unusedTextures) {
            AntiqueAtlas.LOGGER.warn("[Antique Atlas] Tile texture {} isn't referenced by any structure tile provider!",
                    texture.displayId());
        }
    }
