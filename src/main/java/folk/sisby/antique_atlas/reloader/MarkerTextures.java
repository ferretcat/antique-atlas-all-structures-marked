...
public class MarkerTextures extends SinglePreparationResourceReloader<Map<Identifier, MarkerTextures.MarkerTextureMeta>>
    ...
    public static Identifier lol;
    public static MarkerTexture lolol;
    ...
    public MarkerTexture getLandmarkType(LandmarkType<?> type, String variant) {
        return getOrDefault(Identifier.of(type.id().getNamespace(),
                "landmark/type/" + type.id().getPath() + (variant == null ? "" : "/" + variant)));
    }
    ...
    protected Map<Identifier, MarkerTextureMeta> prepare(ResourceManager manager, Profiler profiler) {
        ...
        for (Map.Entry<Identifier, Resource> e : manager
                .findResources("textures/atlas/marker", id -> id.getPath().endsWith(".png")).entrySet()) {
            String path = e.getKey().getPath().substring("textures/atlas/marker/".length(),
                    e.getKey().getPath().length() - ".png".length());
            Identifier id = Identifier.of(e.getKey().getNamespace(), path);

            if (path.equals("custom/point")) {
                int textureWidth = 32;
                ...
                int nearClip = 1;
                int farClip = Integer.MAX_VALUE;
                lolol = MarkerTexture.ofId(id, offsetX, offsetY, textureWidth, textureHeight, mipLevels, nearClip,
                        farClip, false);
                lol = id;
            }
            ...
            try {
                ...
                textureMeta.put(id,
                        metadata.decode(MarkerTextures.MarkerTextureMeta.METADATA).orElse(MarkerTextureMeta.DEFAULT));
            } catch (IOException ex) {
                AntiqueAtlas.LOGGER.error("[Antique Atlas] Failed to access marker texture metadata for {}", e.getKey(),
                        ex);
    ...
    public record MarkerTextureMeta(Optional<Integer> textureWidth, Optional<Integer> textureHeight,
            Optional<Integer> mipLevels, Optional<Integer> offsetX, Optional<Integer> offsetY,
            Optional<Integer> nearClip, Optional<Integer> farClip) {
        public static final MarkerTextureMeta DEFAULT = new MarkerTextureMeta(Optional.empty(), Optional.empty(),
        ...
        public static final ResourceMetadataReader<MarkerTextureMeta> METADATA = new CodecUtil.CodecResourceMetadataSerializer<>(
                CODEC, AntiqueAtlas.id("marker"));
        ...
        public MarkerTexture build(Identifier id, boolean accent) {
            ...
            return MarkerTexture.ofId(id, offsetX, offsetY, textureWidth, textureHeight, mipLevels, nearClip, farClip,
                    accent);
        }
    }
}
